...
async function getData() {
  try {
    const res = await fetch('http://127.0.0.1:8000/api/agendas', { next: { revalidate: 0 } });
    if (!res.ok) return [];
    return res.json();
  } catch (error) {
    return [];
    ...
    <main className="flex-1 flex flex-col py-16 px-6 md:px-12 bg-zinc-50 dark:bg-zinc-950">
      <div className="max-w-6xl mx-auto w-full">
        <div className="mb-12 text-center animate-fade-in-up">
          <h1 className="text-4xl font-bold text-zinc-900 dark:text-white mb-4 capitalize">Agenda Karawang</h1>
          <p className="text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto">
            Jelajahi berbagai agenda menarik yang ada di wilayah Kabupaten Karawang.
          </p>
        </div>
        ...
        {data.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {data.map((item: any, idx: number) => {
              let imgUrl = null;
              if (item.poster) {
                  if (Array.isArray(item.poster) && item.poster.length > 0) {
                      imgUrl = item.poster[0];
              ...
              return (
              <Link href={`/agenda/${item.id}`} key={item.id} className="rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group animate-fade-in-up" style={{ animationDelay: `${idx * 0.1}s` }}>
                ...
                <div className="p-6">
                  <span className="text-xs font-bold text-secondary uppercase tracking-wider">{item.location || 'Umum'}</span>
          ...
          <div className="text-center p-16 bg-white dark:bg-zinc-900 rounded-3xl border border-dashed border-zinc-300 dark:border-zinc-700 shadow-sm">
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-lg">Belum ada data agenda yang ditambahkan.</p>
          </div>
        )}
      </div>
    </main>
