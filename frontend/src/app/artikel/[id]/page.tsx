...
async function getDetail(id: string) {
  try {
    const res = await fetch(`http://127.0.0.1:8000/api/artikels/${id}`, { next: { revalidate: 0 } });
  ...
  if (!data) {
    return (
      <main className="flex-1 flex flex-col py-32 items-center justify-center text-center">
        ...
        <Link href="/artikel" className="text-primary hover:underline">Kembali ke daftar artikel</Link>
      </main>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div className="md:col-span-2">
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{__html: data.content || '<p>Deskripsi belum tersedia.</p>'}}
              />
            </div>
              ...
              <div className="bg-zinc-50 dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
                ...
                <ul className="text-sm">
                  ...
                  <li className="flex flex-col mb-4">
                    <span className="text-zinc-500 dark:text-zinc-400 mb-1">Penulis</span>
                    <span className="font-medium text-zinc-900 dark:text-white">{data.author || '-'}</span>
                  </li>
                  <li className="flex flex-col mb-4">
                  ...
                  <li className="flex flex-col mb-4">
                    <span className="text-zinc-500 dark:text-zinc-400 mb-1">Tanggal Rilis</span>
                    <span className="font-medium text-zinc-900 dark:text-white">{data.published_at || '-'}</span>
                  </li>
                </ul>
              </div>
            </div>
