...
async function getData() {
  try {
    const res = await fetch('http://127.0.0.1:8000/api/budayas', { next: { revalidate: 60 } });
    if (!res.ok) return [];
    return res.json();
  } catch (error) {
      ...
      <div className="max-w-6xl mx-auto w-full">
        ...
        {data.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {data.map((item: any) => (
              <Link href={`/budaya/${item.id}`} key={item.id} className="rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
